 ...
 * 设置为0x00 陀螺仪量程为:卤250 dps     获取到的陀螺仪数据除以131           可以转化为带物理单位的数据，单位为：掳/s
 * 设置为0x08 陀螺仪量程为:卤500 dps     获取到的陀螺仪数据除以65.5          可以转化为带物理单位的数据，单位为：掳/s
 * 设置为0x10 陀螺仪量程为:卤1000dps     获取到的陀螺仪数据除以32.8          可以转化为带物理单位的数据，单位为：掳/s
 * 设置为0x18 陀螺仪量程为:卤2000dps     获取到的陀螺仪数据除以16.4          可以转化为带物理单位的数据，单位为：掳/s

 * ICM20602_ACCEL_CONFIG寄存器
 * 设置为0x00 加速度计量程为:卤2g          获取到的加速度计数据 除以16384      可以转化为带物理单位的数据，单位：g(m/s^2)
 * 设置为0x08 加速度计量程为:卤4g          获取到的加速度计数据 除以8192       可以转化为带物理单位的数据，单位：g(m/s^2)
 * 设置为0x10 加速度计量程为:卤8g          获取到的加速度计数据 除以4096       可以转化为带物理单位的数据，单位：g(m/s^2)
 * 设置为0x18 加速度计量程为:卤16g         获取到的加速度计数据 除以2048       可以转化为带物理单位的数据，单位：g(m/s^2)
*/
/* 初始化icm20607芯片 */
...
void get_icm_attitude(void)
{
    ...
    else
    {
        ...
        // get_attitude_angle(&g_icm20607, &g_attitude, DT); // 四元数算法
        //			if (g_icm_test > 0) //不进去
        //        {
        //            if (g_icm_test == 1)
        //            {
