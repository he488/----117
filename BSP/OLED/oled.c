...
// size: 字体大小
// mode: 0,反色显示;1,正常显示
void OLED_ShowSignedNum(u8 x, u8 y, int num, u8 len, u8 size1, u8 mode)
{
    u8 t, temp, m = 0;
    u8 isNegative = 0;
    ...
    if (isNegative)
    {
        ...
        offset = size1 / 2 + m;  // 计算负号占用的空间
        effectiveLen--;         // 数字部分长度减1
    }
    
    // 显示数字部分
    for (t = 0; t < effectiveLen; t++)
    {
        temp = (num / OLED_Pow(10, effectiveLen - t - 1)) % 10;
        if (temp == 0 && t == 0 && effectiveLen > 1)  // 处理前导零（可选）
        {
            OLED_ShowChar(x + offset + (size1 / 2 + m) * t, y, '0', size1, mode);
        }
        else
        {
            OLED_ShowChar(x + offset + (size1 / 2 + m) * t, y, temp + '0', size1, mode);
        }
    }
    
    // 处理特殊情况：当num=0且len=0时（虽然这种情况不太可能）
    if (num == 0 && len == 0)
    {
